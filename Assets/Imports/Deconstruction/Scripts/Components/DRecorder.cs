...
namespace D
{
    ...
    [AddComponentMenu ("D/D Recorder")]
    
    public class DRecorder : MonoBehaviour
    {
        ...
        [NonSerialized]        List<DRigid> rigids;
        
        // Static
        string recordFolder = "DRecords/";
        ...
        void SetVariables()
        {
            if (mode != AnimatorType.Disabled)
            {
                ...
                if (tmList.Count == 0)
                {
                    Debug.Log ("D Record: " + gameObject.name + " Mode set to " + mode.ToString() + " but object has no children. Mode set to None.", gameObject);
        ...
        void SetModePlay()
        {
            if (mode == AnimatorType.Play)
            {
                ...
                if (controller == null)
                {
                    Debug.Log ("D Record: " + gameObject.name + " Mode set to " + mode.ToString() + " but controller is not defined. Mode set to None.", gameObject);
                    mode = AnimatorType.Disabled;
                    return;
                }
                ...
                {
                    Debug.Log ("D Record: " + gameObject.name + " Mode set to " + mode.ToString() + " but animation clip is not defined. Mode set to None.", gameObject);
                    mode = AnimatorType.Disabled;
                    return;
                }
                ...
                if (hasClip == false)
                {
                    Debug.Log ("D Record: " + gameObject.name + " Mode set to " + mode.ToString() + " but animation clip is not defined in controller. Mode set to None.", gameObject);
                    mode = AnimatorType.Disabled;
                    return;
                }
        ...
        public void StartRecord()
        {
            ...
            DMan.inst.advancedDemolitionProperties.parent       = RFManDemolition.FragmentParentType.GlobalParent;
            DMan.inst.advancedDemolitionProperties.globalParent = transform;
        ...
        void SetRigidRecord()
        {
            ...
            // Get all Rigids
            rigids = gameObject.GetComponentsInChildren<DRigid>().ToList();
        ...
        void PrepareRigidRecord(List<DRigid> rigidList)
        {
            for (int i = 0; i < rigidList.Count; i++)
            {
                    ...
                    rigidList[i].demolitionType == DemolitionType.AwakePrefragment)
                {
                    ...
                    rigidList[i].limitations.desc = new List<DRigid>();
        ...
        void SetRigidPlay()
        {
            ...
            {
                rigids = gameObject.GetComponentsInChildren<DRigid>().ToList();
                foreach (DRigid rigid in rigids)
                {
        ...
        void RigidDemolition(DRigid rigid)
        {
