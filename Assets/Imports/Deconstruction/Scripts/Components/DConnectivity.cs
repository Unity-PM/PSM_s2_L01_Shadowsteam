...
namespace D
{
    ...
    [AddComponentMenu ("D/D Connectivity")]
    
    public class DConnectivity : MonoBehaviour
    {
        ...
        public List<DRigid> rigidList;
        public RFCluster          cluster;
        public int                initShardAmount;
        public int                clsCount;
        public DRigidRoot   rigidRootHost;
        public DRigid       meshRootHost;
        ...
        void SetRuntimeMeshRootHost()
        {
            ...
            {
                meshRootHost = GetComponent<DRigid>();
            ...
            if (meshRootHost != null)
            {
                ...
                if (rigidList.Count == 0)
                {
                    ...
                    if (meshRootHost.activation.con == false)
                        Debug.Log ("D Connectivity: " + name + " object has Rigid host with Mesh Root type but activation By Connectivity is disabled. Enable By Connectivity activation.", gameObject);
                    
                    Debug.Log ("D Connectivity: " + name + " has no objects to check for connectivity. Connectivity disabled.", gameObject);
                    ResetSetup();
                    return;
                }
            }
        ...
        bool WarningCheck()
        {
            ...
            if (meshRootHost != null)
            {
                ...
                if (meshRootHost.objectType != ObjectType.MeshRoot)
                {
                    Debug.Log ("D Connectivity: " + name + " object has Rigid host but it's object type is not Mesh Root. Connectivity disabled.", gameObject);
                    return false;
                }
                ...
                if (meshRootHost.activation.con == false)
                {
                    Debug.Log ("D Connectivity: " + name + " object has Rigid host with Mesh Root type but activation By Connectivity is disabled. ", gameObject);
                    return false;
                }

                if (meshRootHost.simulationType != SimType.Inactive && meshRootHost.simulationType != SimType.Kinematic)
                {
                    Debug.Log ("D Connectivity: " + name + " object has Rigid but simulation type is not Inactive or Kinematic. Connectivity disabled. ", gameObject);
                    return false;
                }
            }
            ...
            if (rigidRootHost != null)
            {
                ...
                {
                    Debug.Log ("D Connectivity: " + name + " object has RigidRoot host but activation By Connectivity is disabled.", gameObject);
                    return false;
                }
                
                if (rigidRootHost.simulationType != SimType.Inactive && rigidRootHost.simulationType != SimType.Kinematic)
                {
                    Debug.Log ("D Connectivity: " + name + " object has RigidRoot but simulation type is not Inactive or Kinematic. Connectivity disabled. ", gameObject);
                    return false;
                }
            }
        ...
        void PrepareCluster()
        {
            ...
            {
                Debug.Log ("D Connectivity: " + name + " object has missing shards. Reset and Editor Setup Connectivity host again.", gameObject);
                cluster            = new RFCluster();
        ...
        // Set rigidList
        void SetRigidListByFragments(List<DRigid> rgList)
        {
            // No targets
            if (rgList.Count == 0)
                return;
        ...
        public static bool InitCachedShardsByRigidList (List<DRigid> rigids, RFCluster cluster)
        {
            // Not initialized
        ...
        // Reinit shard's non serialized fields in case of prefab use
        public static void InitCachedShardsByRigidRoot (DRigidRoot root, RFCluster cluster)
        {
            // Set shards for main cluster
            cluster.shards.Clear();
        ...
        void Clusterize()
        {
            for (int i = 0; i < cluster.childClusters.Count; i++)
            {
                ...
                cluster.childClusters[i].rigid = cluster.childClusters[i].tm.gameObject.AddComponent<DRigid>();
                ...
                // Parent by manager
                if (DMan.inst.parent != null)
                    cluster.childClusters[i].rigid.gameObject.transform.parent = DMan.inst.parent.transform;
        ...
        HashSet<Collider> GetOverlappedColliders (Collider col)
        {
            ...
            {
                Debug.Log ("D Connectivity: " + name + ". Fracture Collider is not supported. Use Box or Sphere trigger collider.", gameObject);
            }
