...
namespace D
{
    ...
    [CustomEditor (typeof(DRigidRoot))]
    public class DRigidRootEditor : Editor
    {
        DRigidRoot root;
        ...
        void UI_Simulation()
        {
            ...
            if (EditorGUI.EndChangeCheck())
                foreach (DRigidRoot scr in targets)
                {
        ...
        static bool ActivatableState(DRigidRoot scr)
        {
        ...
        static bool DemolitionState(DRigidRoot scr)
        {
            if (scr.simulationType == SimType.Inactive || scr.simulationType == SimType.Kinematic)
            ...
            if (Application.isPlaying == true)
            {
                if (root.initialized == false)
                {
                    if (GUILayout.Button ("Initialize", GUILayout.Height (25)))
                        foreach (var targ in targets)
                            if (targ as DRigidRoot != null)
                                if ((targ as DRigidRoot).initialized == false)
                                    (targ as DRigidRoot).Initialize();
                }
                ...
                else
                {
                    if (GUILayout.Button ("Reset Rigid Root", GUILayout.Height (25)))
                            foreach (var targ in targets)
                                if (targ as DRigidRoot != null)
                                    if ((targ as DRigidRoot).initialized == true)
                                        (targ as DRigidRoot).ResetRigidRoot();
                }
                GUILayout.Space (2);
            }
  ...
  //              RigidManUI();
            ...
            EditorGUI.BeginChangeCheck();
            root.initialization = (DRigidRoot.InitType)EditorGUILayout.EnumPopup (gui_mn_ini, root.initialization);
            if (EditorGUI.EndChangeCheck())
                foreach (DRigidRoot scr in targets)
                {
                ...
                if (root.cluster.shards.Count == 0)
                    if (GUILayout.Button ("Editor Setup", GUILayout.Height (25)))
                        foreach (var targ in targets)
                            if (targ as DRigidRoot != null)
                            {
                                (targ as DRigidRoot).EditorSetup();
                                SetDirty (targ as DRigidRoot);
                            }
                ...
                if (root.cluster.shards.Count > 0)
                    if (GUILayout.Button ("Reset Setup", GUILayout.Height (25)))
                        foreach (var targ in targets)
                            if (targ as DRigidRoot != null)
                            {
                                //RFPhysic.DestroyColliders (targ as DRigidRoot);
                                (targ as DRigidRoot).ResetSetup();
                                SetDirty (targ as DRigidRoot);
                            }
...
/*
        ...
        void Activate()
        {
            if (Application.isPlaying == true)
                foreach (var targ in targets)
                    if (targ as DRigidRoot != null)
                        if ((targ as DRigidRoot).simulationType == SimType.Inactive || (targ as DRigidRoot).simulationType == SimType.Kinematic)
                            (targ as DRigidRoot).ac();
        }
