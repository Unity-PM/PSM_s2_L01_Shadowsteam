...
namespace D
{
    ...
    public class RFReset
    {
        ...
        // Rigid 
        public static void ResetRigid (DRigid scr)
        {
        ...
        // Reset if object fading/faded
        public static void ResetFade (DRigid scr)
        {
            // Was excluded
        ...
        // Mesh Root 
        static bool MeshRootReset (DRigid scr)
        {
            // Not mesh root
            ...
            // Reset uny data
            DUnyielding.SetMeshRootUnyState (scr.transform, null);
        ...
        static bool MeshRootCleanup (DRigid scr)
        {
            // Cleanup destroyed/faded fragments
        ...
        public static void RigidRootReset (DRigidRoot scr)
        {
            // Stop all cors in case object restarted
            scr.StopAllCoroutines();
            scr.corState                    = false;
        ...
        static void ResetParentAndTm(DRigidRoot scr)
        {
            // TODO null checks
            for (int i = 0; i < scr.cluster.shards.Count; i++)
            {
        ...
        // Set object type back in case of clustering->demolition
        static void ResetSimType(DRigidRoot scr)
        {
        ...
        // Destroy particles
        static void DestroyRigidRootParticles (DRigidRoot scr)
        {
            if (scr.particleList.Count > 0)
            {
                for (int i = scr.particleList.Count - 1; i >= 0; i--)
        ...
        // Destroy clusters
        static void DestroyClusters (DRigidRoot scr)
        {
            for (int i = 0; i < scr.clusters.Count; i++)
                if (scr.clusters[i].tm != null)
                    DMan.DestroyGo (scr.clusters[i].tm.gameObject);
        ...
        // Reinit demolished mesh object
        public static void ResetMeshDemolition (DRigid scr)
        {
        ...
        static void DestroyFragments (DRigid scr)
        {
            // Destroy fragments    
            if (scr.HasFragments == true)
            {
                ...
                for (int i = scr.fragments.Count - 1; i >= 0; i--)
                {
                    if (scr.fragments[i] != null)
                    {
                        ...
                        DMan.DestroyGo (scr.fragments[i].gameObject);

                        // Destroy root
                        if (scr.fragments[i].rootParent != null)
                        {
                            scr.fragments[i].rootParent.gameObject.SetActive (false);
                            DMan.DestroyGo (scr.fragments[i].rootParent.gameObject);
                        }
                    }
                }
                ...
                DMan.inst.advancedDemolitionProperties.ChangeCurrentAmount (-fragmentNum);
                ...
                if (scr.limitations.desc != null && scr.limitations.desc.Count > 0)
                {
                    ...
                    for (int i = 0; i < scr.limitations.desc.Count; i++)
                    {
                        if (scr.limitations.desc[i] != null)
                        {
                            ...
                            DMan.DestroyGo (scr.limitations.desc[i].gameObject);

                            // Destroy root
                            if (scr.limitations.desc[i].rootParent != null)
                            {
                                scr.limitations.desc[i].rootParent.gameObject.SetActive (false);
                                DMan.DestroyGo (scr.limitations.desc[i].rootParent.gameObject);
                            }
                        }
                    }
                    ...
                    // Subtract amount of deleted fragments
                    DMan.inst.advancedDemolitionProperties.ChangeCurrentAmount (-descendantNum);
                }
            }
        }

        // Destroy particles // TODO send to pool
        static void DestroyRigidParticles (DRigid scr)
        {
            // Destroy debris
            if (scr.HasDebris == true)
                for (int d = 0; d < scr.debrisList.Count; d++)
                    if (scr.debrisList[d].hostTm != null)
                    {
                        ...
                        DMan.DestroyGo (scr.debrisList[d].hostTm.gameObject);
                    }
            ...
            if (scr.HasDust == true)
                for (int d = 0; d < scr.dustList.Count; d++)
                    if (scr.dustList[d].hostTm != null)
                    {
                        ...
                        DMan.DestroyGo (scr.dustList[d].hostTm.gameObject);
                    }
        }
        
        // Fragments need and can be reused
        static void ReuseFragments (DRigid scr)
        {
            // Sub amount
            DMan.inst.advancedDemolitionProperties.ChangeCurrentAmount (-scr.fragments.Count);
        ...
        // Preserve Fragments
        static void PreserveFragments (DRigid scr)
        {
            scr.fragments = null;
            scr.rootChild = null;
            scr.limitations.desc.Clear();
        ...
        // Reinit demolished mesh object
        static void ResetClusterDemolition (DRigid scr)
        {
        ...
        static bool ObjectReuseState (DRigid scr)
        {
            // Mesh Root reset
            if (scr.objectType == ObjectType.MeshRoot)
                return true;
        ...
        // Check fragments reuse state
        static bool FragmentReuseState (DRigid scr)
        {
        ...
        static void RestoreTransform (DRigid scr)
        {
            // Restore tm
            scr.physics.LoadInitTransform (scr.transForm);
        ...
        // Reset sound
        static void ResetSound (DSound scr)
        {
            if (scr != null)
            {
