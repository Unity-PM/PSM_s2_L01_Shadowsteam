...
namespace D
{
    ...
    [CustomEditor (typeof(DShatter))]
    public class DShatterEditor : Editor
    {
        DShatter shat;
        Transform      transForm;
        ...
        void UI_Types()
        {
            ...
            {
                foreach (DShatter scr in targets)
                {
                    scr.type = shat.type;
                    SetDirty (scr);
                }
        ...
        void UI_Fragment()
        {
            if (GUILayout.Button ("Fragment", GUILayout.Height (25)))
            {
                ...
                {
                    if (targ as DShatter != null)
                    {
                        if ((targ as DShatter).interactive == false)
                            (targ as DShatter).Fragment();
                        else
                            (targ as DShatter).InteractiveFragment();
            ...
            if (shat.fragmentsLast.Count > 0) // TODO SUPPORT MASS CHECK
            {
                if (GUILayout.Button ("Fragment to Last", GUILayout.Height (22)))
                {
                    foreach (var targ in targets)
                        if (targ as DShatter != null)
                        {
                            (targ as DShatter).DeleteFragmentsLast (1);
                            (targ as DShatter).resetState = true;
                            (targ as DShatter).Fragment (DShatter.FragLastMode.ToLast);

                            // Scale preview if preview turn on
                            if ((targ as DShatter).previewScale > 0 && (targ as DShatter).scalePreview == true)
                                ScalePreview (targ as DShatter);
                        }
                }
                ...
                if (GUILayout.Button ("    Delete Last    ", GUILayout.Height (22)))
                {
                    foreach (var targ in targets)
                        if (targ as DShatter != null)
                        {
                            (targ as DShatter).DeleteFragmentsLast();
                            (targ as DShatter).resetState = true;
                            (targ as DShatter).ResetScale (0f);
                        }
                }
            }
            ...
            if (shat.fragmentsAll.Count > 0 && shat.fragmentsAll.Count > shat.fragmentsLast.Count)
            {
                if (GUILayout.Button (" Delete All ", GUILayout.Height (22)))
                {
                    foreach (var targ in targets)
                        if (targ as DShatter != null)
                        {
                            (targ as DShatter).DeleteFragmentsAll();
                            (targ as DShatter).resetState = true;
                            (targ as DShatter).ResetScale (0f);
                        }
                }
            ...
            if ((int)shat.type <= 5)
            {
                ...
                if (GUILayout.Button (gui_cn_res))
                {
                    foreach (var targ in targets)
                        if (targ as DShatter != null)
                        {
                            (targ as DShatter).ResetCenter();
                            SetDirty (targ as DShatter);
                        }
                    InteractiveChange();
        ...
        static void ColorPreview (DShatter scr)
        {
        ...
        static void ScalePreview (DShatter scr)
        {
        ...
        void UI_Collider()
        {
            ...
            if (GUILayout.Button (" Remove Colliders "))
            {
                foreach (var targ in targets)
                    if (targ as DShatter != null)
                        RemoveColliders (targ as DShatter);
                SceneView.RepaintAll();
            }
        ...
        static void AddColliders (DShatter scr)
        {
            if (scr.fragmentsLast.Count > 0)
        ...
        static void RemoveColliders (DShatter scr)
        {
            if (scr.fragmentsLast.Count > 0)
                foreach (var frag in scr.fragmentsLast)
                {
        ...
        static void DrawGizmosSelected (DShatter shatter, GizmoType gizmoType)
        {
            // Color preview
