...
namespace D
{
    [AddComponentMenu ("D/D Unyielding")]
    
    public class DUnyielding : MonoBehaviour
    {
        ...
        public static void ClusterSetup (DRigid rigid)
        {
            if (rigid.simulationType == SimType.Inactive || rigid.simulationType == SimType.Kinematic)
            {
                DUnyielding[] unyArray =  rigid.GetComponents<DUnyielding>();
        ...
        static void ClusterOverlap (DUnyielding uny)
        {
        ...
        static int ClusterLayerMask(DRigid rigid)
        {
            int mask = 0;
        ...
        // Set clusterized rigids uny state and mesh root rigids
        public static void MeshRootSetup (DRigid mRoot)
        {
            // Get uny list
            List<DUnyielding> unyList = GetUnyList (mRoot.transform);
        ...
        // Get uny list
        static List<DUnyielding> GetUnyList (Transform tm)
        {
            List<DUnyielding> unyList = tm.GetComponents<DUnyielding>().ToList();
        ...
        // Set uny state for mesh root rigids. Used by Mesh Root. Can be used for cluster shards
        static void SetMeshRootUnyRigidList (DRigid mRoot, DUnyielding uny)
        {
            // Get target mask
        ...
        // Get combined layer mask
        static int MeshRootLayerMask(DRigid mRoot)
        {
            int mask = 0;
            for (int i = 0; i < mRoot.fragments.Count; i++)
                if (mRoot.fragments[i].physics.meshCollider != null)
        ...
        // Set rigid list uny and sim states 
        public static void SetMeshRootUnyState (Transform tm, List<DUnyielding> unyList)
        {
            // Get uny list
            if (unyList == null)
        ...
        // Set unyielding state
        static void SetRigidUnyState (DUnyielding uny)
        {
        ...
        static void SetRigidUnySim (DUnyielding uny)
        {
        ...
        // Set clusterized rigids uny state and mesh root rigids
        public static void ResetMeshRootSetup (DRigid mRoot)
        {
            DUnyielding[] unyList = mRoot.GetComponents<DUnyielding>();
        ...
        public static void  SetUnyieldingFragments (DRigid scr)
        {
            // Not mesh demolition
            ...
            // TODO collect layer mask by all layers -> int finalMask = DUnyielding.ClusterLayerMask(scr);
            int layerMask = 1 << scr.gameObject.layer;
            
            // Overlapped objects: Copy uny, stay kinematic
            DUnyielding[] unyArr   = scr.GetComponents<DUnyielding>();
        ...
        // Set unyielding state
        static void SetRigidUnyState (DRigid rigid, bool uny, bool act)
        {
        ...
        static void CopyUny (DUnyielding source, GameObject target)
        {
            DUnyielding newUny = target.AddComponent<DUnyielding>();
        ...
        // Set uny state for mesh root rigids. Used by Mesh Root. Can be used for cluster shards
        public void GetRigidRootUnyShardList(DRigidRoot rigidRoot)
        {
            // Uny disabled
            if (enabled == false)
                return;
