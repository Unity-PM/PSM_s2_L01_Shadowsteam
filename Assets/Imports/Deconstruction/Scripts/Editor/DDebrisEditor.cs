...
namespace D
{
    ...
    [CustomEditor (typeof(DDebris))]
    public class DDebrisEditor : Editor
    {
        DDebris debris;
            ...
            if (Application.isPlaying == true)
            {
                if (GUILayout.Button ("Emit", GUILayout.Height (25)))
                    foreach (var targ in targets)
                        if (targ as DDebris != null)
                            (targ as DDebris).Emit();

                if (GUILayout.Button ("Clean", GUILayout.Height (25)))
                    foreach (var targ in targets)
                        if (targ as DDebris != null)
                            (targ as DDebris).Clean();
            }
        ...
        void UI_Emit()
        {
            ...
            if (EditorGUI.EndChangeCheck())
                foreach (DDebris scr in targets)
                {
