...
namespace D
{
    ...
    public class RFParticles
    {
        ...
        static DDebris[]            debrisArray;
        static DDust[]              dustArray;
        ...
        public static void SetParticleComponents (DRigid scr)
        {
            if (scr.debrisState > 0)
            {
                // Get all Debris and initialize
                debrisArray = scr.GetComponents<DDebris>();
                if (debrisArray.Length > 0)
                {
                    for (int i = 0; i < debrisArray.Length; i++)
                        debrisArray[i].Initialize();

                    scr.debrisList = new List<DDebris> (debrisArray.Length);
                    for (int i = 0; i < debrisArray.Length; i++)
            ...
            if (scr.dustState > 0)
            {
                ...
                dustArray = scr.GetComponents<DDust>();
                if (dustArray.Length > 0)
                {
                    for (int i = 0; i < dustArray.Length; i++)
                        dustArray[i].Initialize();

                    scr.dustList = new List<DDust> (dustArray.Length);
                    for (int i = 0; i < dustArray.Length; i++)
        ...
        // Set Particle Components: Initialize, collect
        public static void SetParticleComponents(DRigidRoot scr)
        {
        ...
        public static void InitDemolitionParticles(DRigid source)
        { 
        ...
        public static void CreateDemolitionDebris(DRigid rigid)
        {
            List<DDebris> particlesTargets = new List<DDebris>();
        ...
        static void CreateReferenceDemolitionDebris (DDebris debris)
        {
            // Get amount list
            SetRigidDebrisFinalAmount (new List<DDebris>(){debris}, debris.emission.burstType, debris.emission.burstAmount);
        ...
        public static void CreateDemolitionDust(DRigid source)
        {
            List<DDust> particlesTargets = new List<DDust>();
            for (int i = 0; i < source.dustList.Count; i++)
            {
                ...
                if (source.demolitionType == DemolitionType.ReferenceDemolition && source.HasFragments == false)
                {
                    SetDustFinalAmount (new List<DDust>(){source.dustList[i]}, source.dustList[i].emission.burstType, source.dustList[i].emission.burstAmount);
        ...
        public static void CreateActivationParticlesRigid(DRigid rigid)
        {
            // Create debris particles
        ...
        public static void CreateActivationParticlesShard(DRigidRoot root, RFShard shard)
        {
            // RigidRoot shard
            if (shard.rigid == null)
            {
        ...
        public static void CreateDebrisRigid(DDebris target)
        {
            // No particles
            ...
            // Create debris
            DDebris.CreateDebris (target, target.rigid.meshFilter, emitMatIndex, target.pSystem);
        }
        
        // Create single debris particle system for Connected Cluster
        public static void CreateDebrisCluster(DRigid rigid, DDebris debris)
        {
            for (int j = rigid.clusterDemolition.cluster.shards.Count - 1; j >= 0; j--)
            {
                ...
                if (rigid.clusterDemolition.cluster.shards[j].neibShards.Count < rigid.clusterDemolition.cluster.shards[j].nAm)
                {
                    ...
                    else if (rigid.meshRoot != null)
                    {
                        ...
                        // Create debris
                        DDebris.CreateDebris (debris, rigid.clusterDemolition.cluster.shards[j].mf, emitMatIndex, debris.pSystem);
                    }
                }
            }
        }
        ...
        public static void CreateDebrisShard(DRigidRoot root, DDebris debris, RFShard shard)
        {
            // Visibility check
            ...
            // Create debris
            DDebris.CreateDebris (debris, shard.mf, emitMatIndex, debris.pSystem);
        }
        ...
        // Create single dust particle system
        public static void CreateDustRigid(DDust target)
        {
            // No particles
            if (target.amountFinal < target.limitations.minParticles && target.emission.distanceRate == 0)
                return;
            ...
            // Create debris
            DDust.CreateDust (target, target.rigid.meshFilter, emitMatIndex, target.pSystem);
        }
        
        // Create single debris particle system for Connected Cluster
        public static void CreateDustCluster(DRigid rigid, DDust dust)
        {
            for (int j = rigid.clusterDemolition.cluster.shards.Count - 1; j >= 0; j--)
            {
                // If has detached neib shard
                if (rigid.clusterDemolition.cluster.shards[j].neibShards.Count < rigid.clusterDemolition.cluster.shards[j].nAm)
                {
                    ...
                    else if (rigid.meshRoot != null)
                    {
                        ...
                        // Create debris
                        DDust.CreateDust (dust, rigid.clusterDemolition.cluster.shards[j].mf, emitMatIndex, dust.pSystem);
                    }
                }
            }
        }
        ...
        // Create single dust particle system
        public static void CreateDustShard(DRigidRoot root, DDust dust, RFShard shard)
        {
            // Visibility check
            ...
            // Create debris
            DDust.CreateDust (dust, shard.mf, emitMatIndex, dust.pSystem);
        }
        ...
        public static ParticleSystem CreateParticleSystemDebris (DDebris debris, Transform tm)
        {
            // Create ps
            ...
            DMan.inst.particles.DestroyOrReset (debris.pSystem, debris.emission.lifeMax + debris.pSystem.main.duration);

            return debris.pSystem;
        }
        
        // Create host and particle system
        public static ParticleSystem CreateParticleSystemDust (DDust dust, Transform tm)
        {
            // Create ps
            ...
            // Destroy particle system or reset back to pool
            DMan.inst.particles.DestroyOrReset (dust.pSystem, dust.emission.lifeMax + dust.pSystem.main.duration);
        ...
        static ParticleSystem CreateParticleSystem(Transform tm)
        {
            ...
            ParticleSystem ps = DMan.inst.particles.psInst == null
                ? DMan.inst.particles.CreateParticleInstance()
                : DMan.inst.particles.GetPoolObject();
        ...
        // Create single debris particle system
        public static void CreateDebrisImpact(DDebris debris, Vector3 impactPos, Vector3 impactNormal)
        {
            ...
            debris.hostTm.parent     = DMan.inst.transForm;
            ...
            // Create debris
            DDebris.CreateDebris (debris, null, -1, ps);
        }

        // Create single debris particle system
        public static void CreateDustImpact(DDust dust, Vector3 impactPos, Vector3 impactNormal)
        {
            // TODO Visibility check
            ...
            dust.hostTm.parent     = DMan.inst.transForm;
            ...
            // Create debris
            DDust.CreateDust (dust, null, -1, ps);
        }
        ...
        // Detach child particles in case object has child particles and about to be deleted
        static void DetachParticles(DRigid source)
        {
            ...
            if (source.HasDebris == true) 
            {
                for (int i = 0; i < source.debrisList.Count; i++)
                {
                    ...
                    {
                        source.debrisList[i].hostTm.parent     = DMan.inst.transForm;
            ...
            if (source.HasDust == true)
            {
                for (int i = 0; i < source.dustList.Count; i++)
                {
                    ...
                    {
                        source.dustList[i].hostTm.parent     = DMan.inst.transForm;
        ...
        public static void CopyRigidParticles(DRigid source, DRigid target)
        {
            ...
            if (source.HasDebris == true)
            {
                ...
                if (target.debrisList == null)
                    target.debrisList = new List<DDebris>(source.debrisList.Count);
                else
                    target.debrisList.Clear();
                
                // Copy every debris from source to target
                DDebris targetDebris;
                for (int i = 0; i < source.debrisList.Count; i++)
                {
                    targetDebris = target.gameObject.AddComponent<DDebris>();
                    ...
                    if (source.debrisList[i].HasChildren == false)
                        source.debrisList[i].children = new List<DDebris>(source.debrisList.Count);
            ...
            if (source.HasDust == true)
            {
                ...
                if (target.dustList == null)
                    target.dustList = new List<DDust>(source.dustList.Count);
                else
                ...
                for (int i = 0; i < source.dustList.Count; i++)
                {
                    targetDust = target.gameObject.AddComponent<DDust>();
                    targetDust.CopyFrom (source.dustList[i]);
                    targetDust.rigid = target;

                    if (source.dustList[i].HasChildren == false)
                        source.dustList[i].children = new List<DDust>(source.dustList.Count);
        ...
        // Copy debris and dust
        public static void CopyRootMeshParticles(DRigid source, List<DRigid> targets)
        {
            // Clean null debris
            if (source.HasDebris == true)
            ...
            if (source.HasDebris == true)
            {
                for (int d = 0; d < source.debrisList.Count; d++)
                {
                    ...
                    for (int i = 0; i < targets.Count; i++)
                    {
                        ...
                        // Copy
                        DDebris targetDebris = targets[i].gameObject.AddComponent<DDebris>();
                        ...
                        if (targets[i].debrisList == null)
                            targets[i].debrisList = new List<DDebris>(targets.Count);
                        targets[i].debrisList.Add (targetDebris);
                        
                        // Collect debris for parent debris
                        if (source.debrisList[d].children == null)
                            source.debrisList[d].children = new List<DDebris>(targets.Count);
            ...
            if (source.HasDust == true)
            {
                for (int d = 0; d < source.dustList.Count; d++)
                {
                    ...
                    for (int i = 0; i < targets.Count; i++)
                    {
                        ...
                        // Copy
                        DDust targetDust = targets[i].gameObject.AddComponent<DDust>();
                        targetDust.CopyFrom (source.dustList[d]);
                        targetDust.rigid = targets[i];
                        
                        // Collect debris for Rigid
                        if (targets[i].dustList == null)
                            targets[i].dustList = new List<DDust>(targets.Count);
                        targets[i].dustList.Add (targetDust);
                        
                        // Collect debris for parent debris
                        if (source.dustList[d].children == null)
                            source.dustList[d].children = new List<DDust>(targets.Count);
         ...
         // Copy debris and dust
        public static void CopyRigidRootParticles(DRigidRoot source, DRigid target)
        {
            // Copy debris
            if (source.HasDebris == true)
            {
        ...
        // Set renderer
        public static void SetParticleRendererDebris (ParticleSystemRenderer rn, DDebris scr)
        {
            // Common vars
        ...
        // Set renderer
        public static void SetParticleRendererDust(ParticleSystemRenderer rn, DDust scr)
        {
            // Common vars
        ...
        // Get debris hosts
        public static void GetDebrisTargets (List<DDebris> filtered, List<DDebris> targets, float sizeThreshold, int percentage, int pType)
        {
            // Set max amount
            int maxAmount = 0;
            for (int i = 0; i < targets.Count; i++)
        ...
        // Get debris hosts
        public static void GetDustTargets (List<DDust> filtered, List<DDust> targets, float sizeThreshold, int percentage, int pType)
        {
            // Set max amount
            int maxAmount = 0;
            for (int i = 0; i < targets.Count; i++)
        ...
        // Get amount list
        public static void SetRigidDebrisFinalAmount(List<DDebris> targets, BurstType burstType, int burstAmount)
        {
            // No burst
        ...
        // Get amount list
        public static void SetDustFinalAmount(List<DDust> targets, BurstType burstType, int burstAmount)
        {
            // No burst
            if (burstType == BurstType.None)
                for (int i = 0; i < targets.Count; i++)
        ...
        public static bool AmountCheck(DRigid source, int pType)
        {
