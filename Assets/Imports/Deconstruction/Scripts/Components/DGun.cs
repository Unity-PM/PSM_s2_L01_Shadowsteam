...
namespace D
{
    [AddComponentMenu("D/D Gun")]
    
    public class DGun : MonoBehaviour
    {
        ...
        public void Shoot (Vector3 shootPos, Vector3 shootVector)
        {
            ...
            // Affected components
            DRigid     rigidScr = null;
            DRigidRoot rootScr  = null;
            ...
            if (rigid == true)
            {
                ...
                rigidScr = hit.collider.attachedRigidbody == null 
                    ? hit.collider.GetComponent<DRigid>() 
                    : hit.collider.attachedRigidbody.transform.GetComponent<DRigid>();
            ...
            {
                // Get rigid from collider or rigid body
                rootScr = hit.collider.GetComponentInParent<DRigidRoot>();
        ...
        // Impact hit to rigid bodies. Activated inactive, detach clusters
        void ImpactRigid(DRigid rigidScr, RaycastHit hit, Vector3 impactPoint, Vector3 shootVector)
        {
            // Prepare impact list
            ...
            if (radius > 0)
            {
                ...
                DRigid scr;
                List<DRigid> impactRigidList = new List<DRigid>();
                for (int i = 0; i < impactColliders.Length; i++)
                {
                    // Get rigid from collider or rigid body
                    scr = impactColliders[i].attachedRigidbody == null 
                        ? impactColliders[i].GetComponent<DRigid>() 
                        : impactColliders[i].attachedRigidbody.transform.GetComponent<DRigid>();
         ...
         // Impact hit to rigid bodies. Activated inactive, detach clusters
        void ImpactRoot(DRigidRoot rootScr, RaycastHit hit, Vector3 impactPoint, Vector3 shootVector)
        {
            // Prepare impact list
            List<Rigidbody> impactRbList = new List<Rigidbody>();
        ...
        DRigid ApplyDamage (DRigid scr, RaycastHit hit, Vector3 shootPos, Vector3 shootVector, Vector3 impactPoint)
        {
            ...
            if (scr.HasFragments == true)
            {
                ...
                if (dmlHitState == true)
                {
                    if (hit.collider.attachedRigidbody != null)
                        return hit.collider.attachedRigidbody.transform.GetComponent<DRigid>();
                    if (hit.collider != null)
                        return hit.collider.transform.GetComponent<DRigid>();
                }
            }
        ...
        // Impact Debris
        void VfxDebris(List<DDebris> debrisList, Vector3 impactPos, Vector3 impactNormal)
        {
        ...
        // Impact Dust
        void VfxDust(List<DDust> dustList, Vector3 impactPos, Vector3 impactNormal)
        {
        ...
        List<DRigid> ActivationCheck(DRigid scrTarget, Vector3 position)
        {
            // Get rigid list with target object
            List<DRigid> rigidList = new List<DRigid>();
            if (scrTarget != null)
            ...
            if (radius > 0)
            {
                ...
                for (int i = 0; i < colliders.Length; i++)
                {
                    ...
                    DRigid scrRigid = colliders[i].gameObject.GetComponent<DRigid>();
