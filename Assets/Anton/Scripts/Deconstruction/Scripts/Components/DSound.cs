...
namespace D
{
    ...
    [SelectionBase]
    [AddComponentMenu ("D/D Sound")]
    
    public class DSound : MonoBehaviour
    {
        ...
        // Non Serialized
        [NonSerialized] public DRigid     rigid;
        [NonSerialized]        DRigidRoot rigidRoot;
        ...
        void SetComponents()
        {
            ...
            // Set rigidroot
            rigidRoot = GetComponent<DRigidRoot>();
            if (rigidRoot != null)
        ...
        void WarningCheck()
        {
            ...
            if (initialization.enable == false &&
                ...
                if (rigid == null && rigidRoot == null)
                    Debug.Log ("D Sound: " + name + " Warning. Sound component has no attached Rigid or RigidRoot component", gameObject);
            ...
            if (initialization.enable == false &&
                ...
                Debug.Log ("D Sound: " + name + " Warning. All events disabled", gameObject);
            
            // No clips
            if (initialization.enable == true)
                if (initialization.clip == null && initialization.HasClips == false)
                    Debug.Log ("D Sound: " + name + " Warning. Initialization sound has no clips to play", gameObject);
            if (activation.enable == true)
                if (activation.clip == null && activation.HasClips == false)
                    Debug.Log ("D Sound: " + name + " Warning. Activation sound has no clips to play", gameObject);
            if (demolition.enable == true)
                if (demolition.clip == null && demolition.HasClips == false)
                    Debug.Log ("D Sound: " + name + " Warning. Demolition sound has no clips to play", gameObject);
            if (collision.enable == true)
                if (collision.clip == null && collision.HasClips == false)
                    Debug.Log ("D Sound: " + name + " Warning. Collision sound has no clips to play", gameObject);
        }

        // Copy from
        public void CopyFrom (DSound source)
        {
