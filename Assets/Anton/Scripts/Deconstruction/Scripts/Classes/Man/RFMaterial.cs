...
namespace D
{
    ...
    public class RFMaterialPresets
    {
        ...
        public static PhysicsMaterial Material (MaterialType materialType)
        {
            // Crete new material
            if (materialType == MaterialType.HeavyMetal)
                return DMan.inst.materialPresets.heavyMetal.material;
            if (materialType == MaterialType.LightMetal)
                return DMan.inst.materialPresets.lightMetal.material;
            if (materialType == MaterialType.DenseRock)
                return DMan.inst.materialPresets.denseRock.material;
            if (materialType == MaterialType.PorousRock)
                return DMan.inst.materialPresets.porousRock.material;
            if (materialType == MaterialType.Concrete)
                return DMan.inst.materialPresets.concrete.material;
            if (materialType == MaterialType.Brick)
                return DMan.inst.materialPresets.brick.material;
            if (materialType == MaterialType.Glass)
                return DMan.inst.materialPresets.glass.material;
            if (materialType == MaterialType.Rubber)
                return DMan.inst.materialPresets.rubber.material;
            if (materialType == MaterialType.Ice)
                return DMan.inst.materialPresets.ice.material;
            if (materialType == MaterialType.Wood)
                return DMan.inst.materialPresets.wood.material;
            return DMan.inst.materialPresets.concrete.material;
        }
    }
}