...
{
    [SelectionBase]
    [AddComponentMenu ("D/D Debris")]
    
    public class DDebris : MonoBehaviour
    {
        // UI
        ...
        [NonSerialized] public List<DDebris> children;
        ...
        // Copy from
        public void CopyFrom(DDebris source)
        {
        ...
        public static void CreateDebris(DDebris scr, MeshFilter emitMeshFilter, int emitMatIndex, ParticleSystem ps)
        {
            // Set main module
        ...
        void SetReferenceMeshes(GameObject refs)
        {
            ...
            if (refs == null)
            {
                Debug.Log ("D Debris: " + gameObject.name + ": Debris reference not defined.", gameObject);
                return;
            }
            ...
            if (mar.Length == 0)
            {
                Debug.Log ("D Debris: " + gameObject.name + ": Debris reference mesh is not defined.", gameObject);
                return;
            }
            ...
            for (int i = 0; i < mar.Length; i++)
            {
                ...
                if (mar[i].sharedMesh != null && mar[i].sharedMesh.vertexCount > 3)
                    ...
                    Debug.Log ("D Debris: " + mar[i].name + ": has no mesh or amount of vertices too low.", gameObject);
            }
