...
namespace D
{
    ...
    public class RFDemolitionSkin
    {
        ...
        public void SetupSkin(DRigid rigid)
        {
    ...
    public class RFDemolitionMesh
    {
        ...
        public static bool DemolishMesh (DRigid scr)
        {
            // Object demolition
            ...
            if (scr.HasFragments == true)
            {
                ...
                // Set parent
                DMan.SetParentByManager (scr.rootChild.transform);
        ...
        public static bool ClusterizeFragments (DRigid rigid)
        {
            // Clusterize disabled
            ...
            DRigid scr = rigid.rootChild.gameObject.AddComponent<DRigid>();
            ...
            {
                DUnyielding[] unyList = rigid.GetComponents<DUnyielding>();
                if (unyList.Length > 0)
                {
                    for (int i = 0; i < unyList.Length; i++)
                    {
                        // scr.gameObject.AddComponent<DUnyielding>();
        ...
        static void MeshesToObjects(DRigid scr, List<DRigid> scrArray, string baseName)
        {
            // Tag and layer
            ...
            for (int i = 0; i < scr.meshes.Length; ++i)
            {
                ...
                DRigid rfScr = DMan.inst.fragments.rgInst == null
                    ? DMan.inst.fragments.CreateRigidInstance()
                    : DMan.inst.fragments.GetPoolObject(DMan.inst.transForm);
                ...
                // Shadow casting
                if (DMan.inst.advancedDemolitionProperties.sizeThreshold > 0 && 
                    DMan.inst.advancedDemolitionProperties.sizeThreshold > scr.meshes[i].bounds.size.magnitude)
        ...
        static List<DRigid> CreateFragments (DRigid scr)
        {
            // Fragments list
            List<DRigid> scrArray = new List<DRigid>(scr.meshes.Length);

            // Stop if has no any meshes
            if (scr.meshes == null)
                return scrArray;
            
            // Create D manager if not created
            DMan.DManInit();
            ...
            // Set root to manager
            DMan.SetParentByManager (scr.rootChild);
        ...
        // SLice mesh
        public static bool SliceMesh(DRigid scr)
        {
            // Empty lists
            scr.DeleteCache();
            scr.DeleteFragments();
            ...
            DUnyielding.SetUnyieldingFragments (scr);
        ...
        static List<DRigid> CreateSlices (DRigid scr)
        {
            // Fragments list
            List<DRigid> scrArray = new List<DRigid>(scr.meshes.Length);
        ...
        // Set custom fragment simulation type if not inherited
        static void SetFragmentSimulationType (DRigid frag, DRigid scr)
        {
        ...
        static void CacheRuntime (DRigid scr)
        {
        ...
        static void CacheInstant (DRigid scr)
        {
            // Input mesh, setup
        ...
        public static void Awake(DRigid scr)
        {
            // Not mesh
        ...
        // PreCache meshes
        static void PreCache(DRigid scr)
        {
        ...
        // Predefine fragments
        static void Prefragment(DRigid scr)
        {
            // Delete existing
            scr.DeleteFragments();
        ...
        public static void ChildrenToFragments(DRigid scr)
        {
            // Not for clusters
            ...
            for (int i = childCount - 1; i >= 0; i--)
            {
                ...
                DRigid childScr = child.GetComponent<DRigid>();

                // Add new if has no. Copy properties
                if (childScr == null)
                {
                    childScr = child.gameObject.AddComponent<DRigid>();
                    childScr.initialization = DRigid.InitType.ByMethod;
                    scr.CopyPropertiesTo (childScr);
                    
                    // Enable use shatter
                    childScr.meshDemolition.sht = child.GetComponent<DShatter>();
