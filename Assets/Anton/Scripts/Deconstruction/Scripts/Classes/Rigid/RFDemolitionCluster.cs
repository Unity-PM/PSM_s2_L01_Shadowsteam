...
namespace D
{
    ...
    public class RFDemolitionCluster
    {
        ...
        public static void ResetClusterize (DRigid scr)
        {
        ...
        // Editor Clusterize
        public static void ClusterizeEditor (DRigid scr)
        {
            // Reset
            ResetClusterize (scr);
        ...
        // Clusterize
        public static void Clusterize (DRigid scr)
        {
            ...
            if (scr.transForm.childCount == 0)
            {
                ...
                Debug.Log ("D Rigid: " + scr.name + " has no children with mesh. Object Excluded from simulation.", scr.gameObject);
                return;
            }
        ...
        static void ClusterizeConnected (DRigid scr)
        {
        ...
        static void ClusterizeNested (DRigid scr)
        {
        ...
        static void ClusterizeNestedRecursive(DRigid scr, Transform transform, RFCluster cluster, ConnectivityType connectivity)
        {
            ...
            if (tmClusters.Count > 0)
            {
                for (int i = tmClusters.Count - 1; i >= 0; i--)
                {
                    ...
                    // Other properties
                    newCluster.rigid       = newCluster.tm.GetComponent<DRigid>();
        ...
        public static bool DemolishCluster (DRigid scr)
        {
        ...
        public static List<RFShard> DemolishConnectedCluster (DRigid scr, Collider[] detachColliders = null)
        {
        ...
        // Get colliders to detach
        static Collider[] GetDetachColliders (DRigid scr)
        {
        ...
        static List<RFShard> DetachShardsByColliders (DRigid scr, Collider[] detachColliders)
        {
        ...
        static void DemolishNestedCluster (DRigid scr)
        {
        ...
        public static void PostDemolitionCluster (DRigid scr, List<RFShard> detachShards)
        {
            // Prepare
            if (scr.fragments == null) 
                scr.fragments = new List<DRigid>();
            else 
                scr.fragments.Clear();
        ...
        // Create runtime clusters
        static void SetupDetachShards (DRigid scr, List<RFShard> detachShards)
        {
            // No shards to create
            if (detachShards.Count == 0)
                return;
        ...
        // Create Rigid Child cluster
        static void CreateChildClusters (DRigid scr, List<RFCluster> childClusters)
        {
        ...
        static void UpdateOriginalCluster (DRigid scr)
        {
        ...
        public static void SliceConnectedCluster (DRigid scr)
        {
            // Get detach distance
        ...
        // Setup cluster by one slice plane side shards. Connectivity check
        static void SetupPlaneShards (DRigid scr, List<RFShard> clusterShards, List<RFShard> detachShards)
        {
        ...
        // Demolish connected cluster by colliders
        static void ClusterizeDetachShards (DRigid scr, List<RFShard> detachShards, int clusterAmount, int sortType)
        {
        ...
        // Create runtime clusters
        public static void CreateClusterRuntime (DRigid scr, RFCluster cluster)
        {
            ...
            if (cluster.tm == null)
            {
                ...
                DMan.SetParentByManager (cluster.tm, scr.transForm);
            }
            ...
            if (scr != cluster.rigid)
            {
                ...
                {
                    cluster.rigid = cluster.tm.gameObject.GetComponent<DRigid>();
                    if (cluster.rigid == null)
                        cluster.rigid = cluster.tm.gameObject.AddComponent<DRigid>();
                }
            ...
            if (scr.rigidRoot != null)
            {
                ...
                DMan.inst.storage.Register (cluster.tm);
            }
        ...
        static void AddRigidComponent (DRigid scr, List<RFShard> shardList)
        {
            // Get parent for connected cluster detached shards 
            Transform tm = DMan.GetParentByManager (scr);
        ...
        // Add rigid component to transform list
        static void AddRigidComponent (DRigid scr, RFShard shard, Transform parent)
        {
            // Set parent
            ...
            {
                // Add Rigid
                shard.rigid = shard.tm.gameObject.AddComponent<DRigid>();
        ...
        static void SetRigidRootShard(DRigid scr, List<RFShard> shards)
        {
        ...
        // All shards should be clusterized to one cluster. Stop
        static bool SameClusterCheck(DRigid scr, List<RFShard> detachShards, int shardAmount, int clusterAmount)
        {
