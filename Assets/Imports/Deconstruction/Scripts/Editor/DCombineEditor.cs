...
namespace D
{
    ...
    [CustomEditor (typeof(DCombine))]
    public class DCombineEditor : Editor
    {
        DCombine     combine;
        ...
        void UI_Source()
        {
            ...
            combine.type = (DCombine.CombType)EditorGUILayout.EnumPopup (gui_sourceType, combine.type);
            if (EditorGUI.EndChangeCheck() == true)
            {
                foreach (DCombine scr in targets)
                {
            ...
            if (combine.type == DCombine.CombType.ObjectsList)
            {
                GUILayout.Space (space);
