...
namespace D
{
    public class RFCombineMesh
    {
        ...
        public static Mesh CombineShatter (DShatter shatter, Transform root, List<MeshFilter> filters)
        {
            // Get meshes and tms
    ...
    [AddComponentMenu("D/D Combine")]
    
    public class DCombine : MonoBehaviour
    {
        ...
        bool SetData()
        {
            ...
            if (particleRends != null && particleRends.Count > 0)
            {
                ...
                {
                    Mesh m = new Mesh();
                    pr.BakeMesh (m, ParticleSystemBakeMeshOptions.BakeRotationAndScale | ParticleSystemBakeMeshOptions.BakePosition);
            ...
            if (totalVerts >= 65535)
            {
                Debug.Log ("D Combine: " + name + " combined mesh has more than 65535 vertices. UInt32 mesh Index Format will be used.", gameObject);
