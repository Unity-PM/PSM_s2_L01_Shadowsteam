...
namespace D
{
    ...
    [CustomEditor (typeof(DCluster))]
    public class DClusterEditor : Editor
    {
        // Draw gizmo
        [DrawGizmo (GizmoType.Selected | GizmoType.NonSelected | GizmoType.Pickable)]
        static void DrawGizmosSelected (DCluster cluster, GizmoType gizmoType)
        {
            // Color preview
        ...
        {
            // Get cluster
            DCluster cluster = target as DCluster;
        ...
        // Color preview
        static void ColorPreview (DCluster cluster)
        {
            Random.InitState (1);
        ...
        void ScalePreview (DCluster cluster)
        {
            // Scale cluster roots
            if (cluster.allClusters.Count > 0)
            {
        ...
        void ResetScale (DCluster cluster, float scaleValue)
        {
            // Rest scale
        ...
        // Select shards
        static void SelectShards (DCluster cluster)
        {
        ...
        // Get max depth
        static int MaxLayer (DCluster cluster)
        {
            int depth = 0;
            foreach (var cls in cluster.allClusters)
                if (cls.depth > depth)
