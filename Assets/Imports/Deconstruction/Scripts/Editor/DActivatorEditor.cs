...
namespace D
{
    ...
    [CustomEditor (typeof(DActivator))]
    public class DActivatorEditor : Editor
    {
        DActivator         activator;
        ...
        void UI_Components()
        {
            ...
            if (EditorGUI.EndChangeCheck())
                foreach (DActivator scr in targets)
                {
        ...
        void UI_Gizmo()
        {
            ...
            if (activator.gizmoType == DActivator.GizmoType.Box ||
                activator.gizmoType == DActivator.GizmoType.Sphere)
            {
                GUILayout.Space (space);
                UI_GizmoShow();
            }
        ...
        {
            EditorGUI.BeginChangeCheck();
            activator.gizmoType = (DActivator.GizmoType)EditorGUILayout.EnumPopup (gui_gizmoType, activator.gizmoType);
        ...
        void UI_Activation()
        {
            ...
            if (activator.gizmoType != DActivator.GizmoType.ParticleSystem)
            {
                UI_ActivationType();
        ...
        {
            EditorGUI.BeginChangeCheck();
            activator.type = (DActivator.ActivationType)EditorGUILayout.EnumPopup (gui_activationType, activator.type);
            if (EditorGUI.EndChangeCheck() == true)
                foreach (DActivator scr in targets)
                {
        ...
        {
            EditorGUI.BeginChangeCheck();
            activator.positionAnimation = (DActivator.AnimationType)EditorGUILayout.EnumPopup (gui_animPosition, activator.positionAnimation);
            if (EditorGUI.EndChangeCheck() == true)
                SetDirty (activator);
        }
        ...
        void UI_AnimationAddRemoveClear()
        {
            ...
            {
                if (activator.positionAnimation == DActivator.AnimationType.ByGlobalPositionList ||
                    activator.positionAnimation == DActivator.AnimationType.ByLocalPositionList)
                {
                    GUILayout.Space (space);
                    EditorGUI.BeginChangeCheck();
                    GUILayout.BeginHorizontal();
            ...
            if (Application.isPlaying == true)
            {
                ...
                if (GUILayout.Button ("Start", GUILayout.Height (25)))
                    foreach (var targ in targets)
                        if (targ as DActivator != null)
                            (targ as DActivator).TriggerAnimation();
                
                if (GUILayout.Button ("Stop", GUILayout.Height (25)))
                    foreach (var targ in targets)
                        if (targ as DActivator != null)
                            (targ as DActivator).StopAnimation();
                
                if (GUILayout.Button ("Reset", GUILayout.Height (25)))
                    foreach (var targ in targets)
                        if (targ as DActivator != null)
                            (targ as DActivator).ResetAnimation();
        ...
        static void DrawGizmosSelected (DActivator targ, GizmoType gizmoType)
        {
        ...
        void OnSceneGUI()
        {
            activator = target as DActivator;
            if (activator == null)
                return;
        ...
        static void DrawGizmo (DActivator targ)
        {
            // Gizmo properties
            ...
            // Box gizmo
            if (targ.gizmoType == DActivator.GizmoType.Box)
                Gizmos.DrawWireCube (Vector3.zero, targ.boxSize);

            // Sphere gizmo
            if (targ.gizmoType == DActivator.GizmoType.Sphere)
            {
                // Vars
                int   size   = 45;
                float scale  = 1f / size;
