...
namespace D
{
    ...
    [CustomEditor (typeof(DConnectivity))]
    public class DConnectivityEditor : Editor
    {
        DConnectivity conn;
        ...
        void UI_Preview()
        {
            ...
            if (EditorGUI.EndChangeCheck() == true)
                foreach (DConnectivity scr in targets)
                {
        ...
        void UI_Collapse()
        {
            ...
            EditorGUI.BeginChangeCheck();
            conn.startCollapse = (DConnectivity.RFConnInitType)EditorGUILayout.EnumPopup (gui_clp_init, conn.startCollapse);
            if (EditorGUI.EndChangeCheck() == true)
                foreach (DConnectivity scr in targets)
                {
            ...
            if (conn.startCollapse == DConnectivity.RFConnInitType.ByIntegrity)
            {
                GUILayout.Space (space);
        ...
        void UI_Collapse_Buttons()
        {
            ...
            if (conn.collapse.inProgress == false)
            {
                if (GUILayout.Button ("Start Collapse", GUILayout.Height (25)))
                    foreach (var targ in targets)
                        if (targ as DConnectivity != null)
                            RFCollapse.StartCollapse (targ as DConnectivity);
            }
            ...
            if (conn.collapse.inProgress == true)
            {
                if (GUILayout.Button ("Stop Collapse", GUILayout.Height (25)))
                    foreach (var targ in targets)
                        if (targ as DConnectivity != null)
                            RFCollapse.StopCollapse (targ as DConnectivity);
            }
        }
        ...
        void UI_Stress()
        {
            ...
            EditorGUI.BeginChangeCheck();
            conn.startStress = (DConnectivity.RFConnInitType)EditorGUILayout.EnumPopup (gui_str_init, conn.startStress);
            if (EditorGUI.EndChangeCheck() == true)
                foreach (DConnectivity scr in targets)
                {
            ...
            if (conn.startStress == DConnectivity.RFConnInitType.ByIntegrity)
            {
                GUILayout.Space (space);
            ...
            if (Application.isPlaying == true)
            {
                ...
                if (conn.stress.inProgress == false)
                {
                    if (GUILayout.Button ("Start Stress ", GUILayout.Height (25)))
                        foreach (var targ in targets)
                            if (targ as DConnectivity != null)
                                RFStress.StartStress (targ as DConnectivity);
                }
                ...
                if (conn.stress.inProgress == true)
                {
                    if (GUILayout.Button ("Stop Stress", GUILayout.Height (25)))
                        foreach (var targ in targets)
                            if (targ as DConnectivity != null)
                                RFStress.StopStress (targ as DConnectivity);
                }
            }
        }
        ...
        static void StressDraw (DConnectivity targ)
        {
        ...
        static void DrawGizmosSelected (DConnectivity targ, GizmoType gizmoType)
        {
            // Draw bounding gizmo
            GizmoDraw (targ);
            ...
            // if (targ.cluster.shards.Count > 0)
            {
                if (RFCluster.IntegrityCheck (targ.cluster) == false)
                    Debug.Log ("D Connectivity: " + targ.name + " has missing shards. Reset or Setup cluster.", targ.gameObject);
                else
        ...
        static void GizmoDraw (DConnectivity targ)
        {
        ...
        void SetDirty (DConnectivity scr)
        {
            if (Application.isPlaying == false)
            {
