...
namespace D
{
    ...
    [CustomEditor (typeof(DUnyielding))]
    public class DUnyieldingEditor : Editor
    {
        DUnyielding uny;
        ...
        static void DrawGizmosSelected (DUnyielding targ, GizmoType gizmoType)
        {
            ...
            if (Application.isPlaying == true)
                if (GUILayout.Button ("   Activate   ", GUILayout.Height (25)))
                    foreach (var targ in targets)
                        if (targ as DUnyielding != null)
                            (targ as DUnyielding).Activate();
        ...
        void UI_Gizmo()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                foreach (DUnyielding scr in targets)
                {
        ...
        void UI_Properties()
        {
            ...
            EditorGUI.BeginChangeCheck();
            uny.simulationType = (DUnyielding.UnySimType)EditorGUILayout.EnumPopup (gui_propSim, uny.simulationType);
            if (EditorGUI.EndChangeCheck() == true)
            {
                foreach (DUnyielding scr in targets)
                {
