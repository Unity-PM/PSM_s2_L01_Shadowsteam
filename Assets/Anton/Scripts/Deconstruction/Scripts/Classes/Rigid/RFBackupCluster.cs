...
namespace D
{
    public class RFBackupCluster
    {
        ...
        public static void BackupConnectedCluster (DRigid scr)
        {
            // No need to save
        ...
        public static void ResetRigidCluster (DRigid scr)
        {
            if (scr.reset.action == RFReset.PostDemolitionType.DeactivateToReset)
            {
        ...
        // Remove particles
        static void DestroyParticles(DRigid scr)
        {
        ...
        static void ResetDeepShardRigid (DRigid scr, RFCluster cluster)
        {
        ...
        static void ResetShardRigid (RFShard shard)
        {
            ...
            // Get shard's rigid
            if (shard.rigid == null)
                shard.rigid = shard.tm.GetComponent<DRigid>();
            ...
            if (cluster.HasChildClusters == true)
            {
                for (int i = 0; i < cluster.childClusters.Count; i++)
                {
                    ...
                    if (cluster.childClusters[i].rigid == null)
                        cluster.childClusters[i].rigid = cluster.childClusters[i].tm.GetComponent<DRigid>();
        ...
        static void DestroyRoots (DRigid scr)
        {
        ...
        static bool BackupState (DConnectivity scr, bool warning = false)
        {
            ...
            {
                if (warning == true)
                    Debug.Log ("D Rigid: " + scr.name + " can not reset Connectivity. Enable Connectivity Reset in Reset properties", scr.gameObject);
                return false;
            }
            ...
            {
                if (warning == true)
                    Debug.Log ("D Rigid Root: " + scr.name + " can not reset Connectivity. Enable Connectivity Reset in Reset properties", scr.gameObject);
                return false;
            }
        ...
        // Save backup cluster to restore it later
        public static void BackupConnectivity (DConnectivity scr)
        {
            // Checks state
        ...
        // Restore cluster using backup cluster
        public static void RestoreConnectivity (DConnectivity scr)
        {
            // Checks state
            ...
            if (scr.meshRootHost != null)
            {
                ...
                if (DConnectivity.InitCachedShardsByRigidList (scr.rigidList, scr.cluster) == true)
            ...
            if (scr.rigidRootHost != null)
            {
                ...
                // Shards were cached, reinit non serialized vars, clear list otherwise
                DConnectivity.InitCachedShardsByRigidRoot (scr.rigidRootHost, scr.cluster);
