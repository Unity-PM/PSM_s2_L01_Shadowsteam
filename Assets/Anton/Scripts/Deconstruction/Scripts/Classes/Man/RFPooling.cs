...
namespace D
{
    ...
    public class RFPoolingFragment
    {
        ...
        [NonSerialized]        DRigid        rg;
        [NonSerialized]        Rigidbody           rb;
        [NonSerialized] public DRigid        rgInst;
        [NonSerialized] public Queue<DRigid> queue;
        [NonSerialized] public bool                inProgress;
        ...
        // Create pool object
        public DRigid CreateRigidInstance()
        {
        ...
        // Get pool object
        public DRigid GetPoolObject (Transform manTm)
        {
            if (enable == true)
            {
                while (queue.Count > 0)
        ...
        // Create pool object
        DRigid CreatePoolObject (Transform manTm)
        {
            // Create instance if null
            if (rgInst == null)
        ...
        public void DestroyOrReset(DRigid rgBack, float lifeTime)
        {
            // Destroy if backpooling disabled or max capacity reached
            if (reuse == false || queue.Count > maxCap)
            {
        ...
        void RigidPoolReset(DRigid rgBack)
        {
            // Set tm
        ...
        public static void GlobalReset(DRigid scr)
        {
            scr.initialization = DRigid.InitType.ByMethod;
