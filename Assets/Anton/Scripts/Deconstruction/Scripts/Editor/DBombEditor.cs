...
namespace D
{
    ...
    [CustomEditor (typeof(DBomb))]
    public class DBombEditor : Editor
    {
        DBomb        bomb;
        ...
        static void DrawGizmosSelected (DBomb bomb, GizmoType gizmoType)
        {
        ...
        void UI_Actions()
        {
            if (Application.isPlaying == true)
            {
                ...
                {
                    foreach (DBomb script in targets)
                    {
        ...
        {
            EditorGUI.BeginChangeCheck();
            bomb.rangeType = (DBomb.RangeType)EditorGUILayout.EnumPopup (gui_rangeType, bomb.rangeType);
            if (EditorGUI.EndChangeCheck() == true)
            {
                foreach (DBomb scr in targets)
                {
        ...
        void UI_Impulse()
        {
            ...
            if (bomb.fadeType == DBomb.FadeType.ByCurve)
            {
                GUILayout.Space (space);
        ...
        {
            EditorGUI.BeginChangeCheck();
            bomb.fadeType = (DBomb.FadeType)EditorGUILayout.EnumPopup (gui_rangeFade, bomb.fadeType);
            if (EditorGUI.EndChangeCheck() == true)
            {
                foreach (DBomb scr in targets)
                {
        ...
        static void DrawGizmo (DBomb bomb)
        {
            // Vars
            float       rate          = 0f;
            const int   size          = 45;
