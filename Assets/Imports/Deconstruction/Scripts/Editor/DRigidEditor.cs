...
namespace D
{
    ...
    [CustomEditor (typeof(DRigid), true)]
    public class DRigidEditor : Editor
    {
        DRigid rigid;
        ...
        static GUIContent gui_phy_mtp     = new GUIContent ("Type",               "Material preset with predefined density, friction, elasticity and solidity. Can be edited in D Man component.");
        ...
        static GUIContent gui_act_act     = new GUIContent ("Activator",          "Inactive object will be activated by overlapping with object with D Activator component.");
        static GUIContent gui_act_imp     = new GUIContent ("Impact",             "Inactive object will be activated when it will be shot by DGun component.");
        ...
        static GUIContent gui_lim_slc     = new GUIContent ("Slice By Blade",     "Allows object to be sliced by object with D Blade component.");
        ...
        static GUIContent gui_msh_sh      = new GUIContent ("Use Shatter",        "Allows to use D Shatter properties for fragmentation. Works only if object has D Shatter component.");
        ...
        static GUIContent gui_ref_add    = new GUIContent ("Add Rigid",            "Add D Rigid component to reference with mesh.");
        ...
        void UI_Main()
        {
            ...
            rigid.initialization = (DRigid.InitType)EditorGUILayout.EnumPopup (gui_mn_ini, rigid.initialization);
            if (EditorGUI.EndChangeCheck())
                foreach (DRigid scr in targets)
                {
        ...
        static bool ActivatableState(DRigid scr)
        {
        ...
        static bool DemolishableState(DRigid scr)
        {
        ...
        static bool MeshState(DRigid scr)
        {
            ...
            if (Application.isPlaying == true)
            {
                if (rigid.initialized == false)
                {
                    if (GUILayout.Button ("Initialize", GUILayout.Height (25)))
                        foreach (var targ in targets)
                            if (targ as DRigid != null)
                                if ((targ as DRigid).initialized == false)
                                    (targ as DRigid).Initialize();
                }
                ...
                else
                {
                    if (GUILayout.Button ("Reset Rigid", GUILayout.Height (25)))
                            foreach (var targ in targets)
                                if (targ as DRigid != null)
                                    if ((targ as DRigid).initialized == true)
                                        (targ as DRigid).ResetRigid();
                }
            }
        ...
        void InfoUI()
        {
            ...
            if (Application.isPlaying == true && rigid.enabled == true && rigid.initialized == true && rigid.objectType != ObjectType.MeshRoot)
            {
                ...
                // Bad mesh
                if (rigid.meshDemolition.badMesh > DMan.inst.advancedDemolitionProperties.badMeshTry)
        ...
        void Activate()
        {
            if (Application.isPlaying == true)
                foreach (var targ in targets)
                    if (targ as DRigid != null)
                        if ((targ as DRigid).simulationType == SimType.Inactive || (targ as DRigid).simulationType == SimType.Kinematic)
                            (targ as DRigid).Activate();
        }
        ...
        void SetupUI()
        {
            ...
            if (GUILayout.Button (" Editor Setup ", GUILayout.Height (25)))
                foreach (var targ in targets)
                    if (targ as DRigid != null)
                    {
                        (targ as DRigid).EditorSetup();
                        SetDirty (targ as DRigid); 
                    }
            ...
            if (rigid.objectType == ObjectType.ConnectedCluster)
            {
                ...
                if (GUILayout.Button ("Start Collapse", GUILayout.Height (25)))
                    if (Application.isPlaying)
                        foreach (var targ in targets)
                            if (targ as DRigid != null)
                                RFCollapse.StartCollapse (targ as DRigid);
            }
        }
        
        void ClusterPreviewUI(DRigid scr)
        {
            if (rigid.objectType == ObjectType.ConnectedCluster)
            {
                GUILayout.BeginHorizontal();
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    foreach (Object targ in targets)
                        if (targ as DRigid != null)
                        {
                            (targ as DRigid).clusterDemolition.cn = rigid.clusterDemolition.cn;
                            (targ as DRigid).clusterDemolition.nd = rigid.clusterDemolition.nd;
                            SetDirty (targ as DRigid); 
                        }
        ...
        static void DrawGizmosSelected (DRigid targ, GizmoType gizmoType)
        {
            // Missing shards
        ...
        static void ClusterDraw(DRigid targ)
        {
