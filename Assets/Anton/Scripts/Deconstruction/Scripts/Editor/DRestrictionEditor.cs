...
namespace D
{
    ...
    [CustomEditor (typeof(DRestriction))]
    public class DRestrictionEditor : Editor
    {
        // Target
        DRestriction rest;
        ...
        static GUIContent gui_rigid      = new GUIContent ("D Rigid", "");
        ...
        public override void OnInspectorGUI()
        {
            ...
            rest.rigid = (DRigid)EditorGUILayout.ObjectField (gui_rigid, rest.rigid, typeof(DRigid), true);
            if (EditorGUI.EndChangeCheck())
                foreach (DRestriction scr in targets)
                {
                    SetDirty (scr);
                }
        ...
        void UI_Prop()
        {
            ...
            EditorGUI.BeginChangeCheck();
            rest.breakAction = (DRestriction.RFBoundActionType)EditorGUILayout.EnumPopup (gui_propAction, rest.breakAction);
        ...
        void UI_Dist()
        {
            ...
            EditorGUI.BeginChangeCheck();
            rest.position = (DRestriction.RFDistanceType)EditorGUILayout.EnumPopup (gui_distPos, rest.position);
            if (EditorGUI.EndChangeCheck() == true)
                foreach (DRestriction scr in targets)
                {
            ...
            if (rest.position == DRestriction.RFDistanceType.TargetPosition)
            {
                GUILayout.Space (space);
        ...
        void UI_Trig()
        {
            ...
            EditorGUI.BeginChangeCheck();
            rest.region = (DRestriction.RFBoundTriggerType)EditorGUILayout.EnumPopup (gui_trigReg, rest.region);
            if (EditorGUI.EndChangeCheck() == true)
                foreach (DRestriction scr in targets)
                {
        ...
        void SetDirty (DRestriction scr)
        {
