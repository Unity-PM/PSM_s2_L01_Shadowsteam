...
namespace D
{
    ...
    public class RFPhysic
    {
        ...
        public static void SetSimulationType(Rigidbody rb, SimType simulationType, ObjectType objectType, bool useGravity, int solver, float sleep = 0.005f)
        {
            ...
            // Properties
            rb.interpolation            = DMan.inst.interpolation;
        ...
        static void SetCollisionDetection(Rigidbody rb, ObjectType objectType)
        {
            if (objectType == ObjectType.NestedCluster || objectType == ObjectType.ConnectedCluster)
                rb.collisionDetectionMode = DMan.inst.clusterCollision;
            else
                rb.collisionDetectionMode = DMan.inst.meshCollision;
        }
        ...
        public static void SetDensity(DRigid scr)
        {
            ...
            {
                // scr.physics.rigidBody.SetDensity(DMan.inst.materialPresets.Density(scr.physics.mt));
        ...
        {
            if (DMan.inst.minimumMass > 0)
                if (m < DMan.inst.minimumMass)
                    return DMan.inst.minimumMass;
            if (DMan.inst.maximumMass > 0)
                if (m > DMan.inst.maximumMass)
                    return DMan.inst.maximumMass;
            return m;
        }
        ...
        // Set drag properties
        public static void SetDrag(DRigid scr)
        {
            scr.physics.rigidBody.linearDamping        = DMan.inst.materialPresets.Drag(scr.physics.mt);
            scr.physics.rigidBody.angularDamping = DMan.inst.materialPresets.AngularDrag(scr.physics.mt);
        }
        ...
        // Set velocity
        public static void SetFragmentsVelocity (DRigid scr)
        {
        ...
        public static void SetFragmentCollider(DRigid scr, Mesh mesh)
        {
            // Custom collider
        ...
        // Set fragments collider
        public static void SetRigidCollider (DRigid scr, Mesh mesh = null)
        {
            // Skip collider
            ...
            // Size check
            if (DMan.inst != null && DMan.inst.colliderSize > 0)
                if (scr.meshRenderer.bounds.size.magnitude < DMan.inst.colliderSize)
                    return;
            ...
            if (scr.physics.meshCollider == null)
            {
                ...
                if (scr.physics.ct == RFColliderType.Mesh)
                {
                    ...
                    if (scr.physics.pc == true && scr.meshFilter.sharedMesh.vertexCount < DMan.coplanarVertLimit)
                    {
                        if (RFShatterAdvanced.IsCoplanar (scr.meshFilter.sharedMesh, RFShatterAdvanced.planarThreshold) == true)
                        {
                            if (DMan.debugStatic == true)
                                Debug.Log ("D Rigid: " + scr.name + " had planar low poly mesh. Object can't get Mesh Collider.", scr.gameObject);
                            scr.physics.ct = RFColliderType.None;
                            return;
                        }
                    }
                    ...
                    mCol.cookingOptions = DMan.cookingOptionsStatic;
        ...
        // Set fragments collider
        public static void SetRigidRootCollider (DRigidRoot root, RFPhysic physics, RFShard shard)
        {
            // Get collider
            ...
            if (shard.col == null)
            {
                ...
                if (physics.ct == RFColliderType.Mesh)
                {
                    ...
                    col.cookingOptions = DMan.cookingOptionsStatic;
        ...
        public static void SetupMeshRootColliders(DRigid scr)
        {
            Collider col;
        ...
        public static void SetClusterCollidersByShards (DRigid scr)
        {
        ...
        static void CollidersRemoveNull(DRigid scr)
        {
            if (scr.physics.HasClusterColliders == true)
        ...
        static void SetShardColliders (DRigid scr, RFCluster cluster)
        {
            // Mesh collider
            if (scr.physics.ct == RFColliderType.Mesh)
            {
        ...
        // Check children for mesh or cluster root until all children will not be checked
        static void SetDeepShardColliders (DRigid scr, RFCluster cluster)
        {
            // Set shard colliders
        ...
        public static void CollectClusterColliders (DRigid scr, RFCluster cluster)
        {
        ...
        // Check children for mesh or cluster root until all children will not be checked
        static void CollectDeepColliders (DRigid scr, RFCluster cluster)
        {
        ...
        public static void SetColliderMaterial(DRigid scr)
        {
        ...
        public static void SetParticleColliderMaterial (List<DDebris> debris)
        {
        ...
        public static void SetColliderConvex(DRigid scr)
        {
            if (scr.physics.meshCollider != null)
            {
                // Not Mesh collider
        ...
        public static void DestroyColliders(DRigid scr)
        {
            if (scr.physics.HasClusterColliders == true)
                for (int i = scr.physics.clusterColliders.Count - 1; i >= 0; i--)
                    if (scr.physics.clusterColliders[i] != null)
        ...
        public static void SetPhysics(DRigidRoot root)
        {
        ...
        {
            // Set phys props
            float density     = DMan.inst.materialPresets.Density (physic.mt);
            float drag        = DMan.inst.materialPresets.Drag (physic.mt);
            float dragAngular = DMan.inst.materialPresets.AngularDrag (physic.mt);
        ...
        public static void SetPhysics(RFShard shard, RFPhysic physic)
        {
            ...
            // Set density. After collider defined
            SetDensity (shard, physic, DMan.inst.materialPresets.Density (physic.mt));

            // Set drag properties
            SetDrag (shard, DMan.inst.materialPresets.Drag (physic.mt), DMan.inst.materialPresets.AngularDrag (physic.mt));
        }
        ...
        // Set ignore list
        public static void SetIgnoreColliders(RFPhysic physics, List<DRigid> rigids)
        {
        ...
        // Ignore collision for overlapped shards
        public static void IgnoreNeibCollision(List<DRigid> rigids, List<int> pr)
        {
        ...
        public IEnumerator PhysicsDataCor (DRigid scr)
        {
            // Stop if running 
        ...
        public bool Destructible { get { return DMan.inst.materialPresets.Destructible(mt); } }
        public int  Solidity     { get { return DMan.inst.materialPresets.Solidity(mt); } }
