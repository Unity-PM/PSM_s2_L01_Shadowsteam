...
namespace D
{
    ...
    [CustomEditor (typeof(DGun))]
    public class DGunEditor : Editor
    {
        DGun   gun;
        ...
        void UI_Impact()
        {
            ...
            EditorGUI.BeginChangeCheck();
            gun.type = (DGun.ImpactType)EditorGUILayout.EnumPopup (gui_imp_tp, gun.type);
            if (EditorGUI.EndChangeCheck() == true)
                foreach (DGun scr in targets)
                {
                    scr.type = gun.type;
            ...
            if (gun.type == DGun.ImpactType.AddExplosionForce)
            {
                GUILayout.Space (space);
        ...
        static void DrawGizmosSelected (DGun gun, GizmoType gizmoType)
        {
            // Ray
            if (gun.showRay == true)
            {
