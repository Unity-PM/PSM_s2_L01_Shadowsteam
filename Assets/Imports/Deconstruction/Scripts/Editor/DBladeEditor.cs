...
namespace D
{
    ...
    [CustomEditor (typeof(DBlade))]
    public class DBladeEditor : Editor
    {
        ...
        void UI_Props()
        {
            ...
            blade.actionType = (DBlade.ActionType)EditorGUILayout.EnumPopup (gui_propAction, blade.actionType);
            if (EditorGUI.EndChangeCheck() == true)
            {
                foreach (DBlade scr in targets)
                {
            ...
            EditorGUI.BeginChangeCheck();
            blade.onTrigger = (DBlade.CutType)EditorGUILayout.EnumPopup (gui_propTrigger, blade.onTrigger);
            if (EditorGUI.EndChangeCheck() == true)
            {
                foreach (DBlade scr in targets)
                {
            ...
            if (Application.isPlaying == true && blade.HasTargets == true)
            {
                if (GUILayout.Button (" Slice Target ", GUILayout.Height (25)))
                {
                    foreach (var bl in targets)
                        if (bl as DBlade != null)
                            (bl as DBlade).SliceTarget();
                }
            }
        }
