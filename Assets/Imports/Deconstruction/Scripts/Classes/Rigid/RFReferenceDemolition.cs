...
namespace D
{
    ...
    public class RFReferenceDemolition
    {
        ...
        public static bool DemolishReference (DRigid scr)
        {
            if (scr.demolitionType == DemolitionType.ReferenceDemolition)
            {
                ...
                DRigid refScr = refGo.gameObject.GetComponent<DRigid>();
                ...
                DMan.SetParentByManager (instGo.transform, scr.transForm);
                ...
                DRigid instScr = instGo.gameObject.GetComponent<DRigid>();

                // Reference Root has not rigid. Add to
                if (instScr == null && scr.referenceDemolition.addRigid == true)
                {
                    // Add rigid and copy
                    instScr = instGo.gameObject.AddComponent<DRigid>();
                ...
                if (instScr != null)
                {
                            ...
                            for (int i = 0; i < scr.debrisList.Count; i++)
                            {
                                if (instScr.debrisList[i] != null)
                                {
                                    if (scr.debrisList[i].children == null)
                                        scr.debrisList[i].children = new List<DDebris>();
                                    else
                            ...
                            for (int i = 0; i < scr.dustList.Count; i++)
                            {
                                if (instScr.dustList[i] != null)
                                {
                                    if (scr.dustList[i].children == null)
                                        scr.dustList[i].children = new List<DDust>();
                                    else
                    ...
                    else if (instScr.objectType == ObjectType.Mesh || instScr.objectType == ObjectType.SkinnedMesh)
                    {
                        ...
                        // Destroy instance
                        DMan.DestroyFragment (instScr, instScr.rootParent, 1f);
                    }
        ...
        static GameObject GetInstance (DRigid scr, GameObject refGo)
        {
            GameObject instGo;
        ...
        static void InheritMaterials (DRigid scr, GameObject instGo)
        {
