...
namespace D
{
    ...
    [CustomEditor (typeof(DMan))]
    public class DManEditor : Editor
    {
        Texture2D  logo;
        Texture2D  icon;
        DMan  man;
        ...
        static GUIContent gui_pl_frg   = new GUIContent ("Fragments",        "Create gameobjects with MeshFilter, MeshRenderer and DRigid components until Min Capacity value will be reached. Objects will be used for runtime fragments when needed");
        ...
        void UI_About()
        {
            ...
            GUILayout.Label ("Plugin build: " + DMan.buildMajor + '.' + DMan.buildMinor.ToString ("D2"));

            GUILayout.Space (space);

            // Logo TODO remove if component removed
            if (logo == null)
                logo = (Texture2D)AssetDatabase.LoadAssetAtPath ("Assets/D/Info/Logo/logo_small.png", typeof(Texture2D));
            if (logo != null)
