...
namespace D
{
    ...
    [CustomEditor (typeof(DRecorder))]
    public class DRecorderEditor : Editor
    {
        // Target
        DRecorder recorder;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (recorder.mode == DRecorder.AnimatorType.Record)
                UI_Record();
            if (recorder.mode == DRecorder.AnimatorType.Play)
                UI_Play();
        ...
        void UI_Mode()
        {
            ...
            recorder.mode = (DRecorder.AnimatorType)EditorGUILayout.EnumPopup (" ", recorder.mode);
            if (EditorGUI.EndChangeCheck())
                foreach (DRecorder scr in targets)
                {
        ...
        {
            GUILayout.Space (space);
            GUILayout.Label ("  D Rigid", EditorStyles.boldLabel);
            GUILayout.Space (space);
            
            EditorGUI.BeginChangeCheck();
            recorder.rigidAction = (DRecorder.RigidActionType)EditorGUILayout.EnumPopup ("Action", recorder.rigidAction);
            if (EditorGUI.EndChangeCheck())
                foreach (DRecorder scr in targets)
                {
